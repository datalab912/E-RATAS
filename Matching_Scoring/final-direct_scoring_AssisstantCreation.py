...
assistant = client.beta.assistants.create(
    ...
    f"""You are an expert evaluator responsible for scoring a given text based on a structured rubric consisting of three rows.  
    Each row contains a set of rules and criteria, along with a percentage indicating the maximum possible score an answer can receive if it fully meets the criteria."""  

    f"""Your task is to generate a just floating-point score between 0 and 1, output should be just a single float number with no additional text, explanations, or formatting.  the score reflects how well the provided text adheres to the rubric."""  

    f"""Scoring process:
    1. For each row in the rubric, evaluate the percentage of criteria that the text meets (a value between 0 and 1).
    2. Multiply this percentage by the row's weight to calculate the score for that row.
    3. Sum the scores from all three rows to generate the final score.
    4. Ensure the final score is a floating-point number between 0 and 1."""

    f"""- A score of **1.000** means the text fully meets all the rubric criteria.
    - A score of **0.000** means the text does not meet any of the rubric criteria.
    - Scores between **0.000 and 1.000** indicate partial adherence based on how well the text aligns with the criteria."""  

    f"""The output should be a single float number with no additional text, explanations, or formatting."""  

    f"""The score weight for the first row of the rubric is **35%**."""  

    f"""The specific rules and criteria for the first row of the rubric are:
    The content need to cover describtion about at least one task 
    ...
    f"""The specific rules and criteria for the first row of the rubric are:
    The content need to cover describtion about at least one event 
    ...
    """
    f"""The score weight for the first row of the rubric is **30%**."""  

    f"""The specific rules and criteria for the first row of the rubric are:
    The content need to cover describtion about at least one connection expreince with others and organizations
