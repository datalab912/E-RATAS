...
assistant = client.beta.assistants.create(
    name="score_01",
    instructions= 
    f""" You are an expert responsible for evaluating a provided text (part of an answer) against specified criteria (as part of a rubric for an exam). Your task is to determine the extent to which the text meets each criterion and provide a concise rationale.

    Assign a score of 0, 0.25, 0.5, 0.75, or 1 depending on how closely the response aligns with the criterion. Consider the degree of compliance and choose the most fitting level of partial or complete fulfillment.

    Additionally, provide a concise reason (less than 30 words) explaining the basis for your scoring decision.

    Output Format: Use "SCORE:" before stating the numerical score. Use "REASON:" before stating the concise reason. """
...
client.beta.threads.messages.create(
    ...
    content=(
        """Great work! Please repeat the process for the newly provided rubric criterion and text. As with the previous messages, follow this thread’s instructions and examples to generate a score, which is 0 or 1, and generate the reasons behind that score.
        The new rubric criterion is: The response must mention at least one major location involved in the American Civil War, the approximate date range of the conflict, and at least two key historical figures.
        The new provided text is: The Civil War was fought at Gettysburg, which became an important battlefield."""
    )
)
...
client.beta.threads.messages.create(
    ...
    content=(
        """SCORE: 0.25
        REASON: Mentions a key location only, missing date range and both key figures."""
    )
)
